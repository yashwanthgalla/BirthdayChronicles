...
const DepthCarousel: React.FC<DepthCarouselProps> = ({
  ...
  const wheelTimerRef = useRef<ReturnType<typeof setTimeout> | number | null>(null);
  const autoTimerRef = useRef<ReturnType<typeof setInterval> | number | null>(null);
  ...
  const navigateRandom = useCallback(() => {
    ...
    } while (nextIdx === focusRef.current && n > 1);
    setFocus(nextIdx, true);
  }, [setFocus]);
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      posRef.current += step;
      layout(posRef.current);
      if (wheelTimerRef.current) clearTimeout(wheelTimerRef.current as number);
  ...
  }, [autoplay, autoplayDelay, count, navigateBy, navigateRandom]);
